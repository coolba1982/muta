...
function sub_str($str, $length = 0, $append = true)
{
    ...
    {
        $newstr = mb_substr($str, 0, $length, MUTA_CHARSET);
    }
    elseif (function_exists('iconv_substr'))
    {
        $newstr = iconv_substr($str, 0, $length, MUTA_CHARSET);
    }
    else
    {
 ...
 * @return  void
 **/
function muta_header($string, $replace = true, $http_response_code = 0)
{
...
{
    if (MUTA_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return muta_iconv('utf-8', MUTA_CHARSET, $str);
        }
        elseif (is_array($str))
        {
...
function to_utf8_iconv($str)
{
    if (MUTA_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return muta_iconv(MUTA_CHARSET, 'utf-8', $str);
        }
        elseif (is_array($str))
        {
            foreach ($str as $key => $value)
...
function muta_iconv($source_lang, $target_lang, $source_string = '')
{
    static $chs = NULL;
...
function load_config()
{
    ...
    $data = read_static_cache('muta_config');
    if ($data === false)
    {
        ...
        write_static_cache('muta_config', $arr);
    }
    else
    {
        $arr = $data;
